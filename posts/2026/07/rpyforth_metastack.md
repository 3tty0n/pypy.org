...
RPyForth builds on established Forth implementation techniques. It uses
[indirect threaded code](https://gforth.org/manual/Threading.html), where a
colon definition is a sequence of word references dispatched one at a time. It
also uses [stack caching](https://www.complang.tuwien.ac.at/anton/euroforth/ef05/ertl-gregg05.pdf),
keeping the top stack values outside the main stack array.

The RPyForth-specific part is how this stack cache is arranged for the
meta-tracing JIT compiler generated by the PyPy toolchain. The active stack is
split between scalar fields and a small frame that the compiler can track,
while deeper values use a shared spill array. At ordinary word calls, RPyForth
realigns this window so that stack values can remain visible across the call.
We call the complete three-area representation a _metastack_.
